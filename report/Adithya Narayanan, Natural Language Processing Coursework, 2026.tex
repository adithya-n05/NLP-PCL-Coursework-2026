...
For marking clarity, I report exactly two techniques in the required format (Visual/Tabular Evidence, Analysis, Impact Statement). I have included both a table and figure for each technique to allow for easier analysis of the EDA techniques chosen.
...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{>{\raggedright\arraybackslash}X c c c c}
    ...
    \textbf{Final ensemble (retuned on train set)} & -- & -- & \textbf{0.6336} & \textbf{0.250} \\
    \bottomrule
  \end{tabularx}
...
For local error analysis, we inspected the final ensemble predictions on the official dev split (2094 examples) against the gold labels from \texttt{dev\_semeval\_parids-labels.csv}. This follows the coursework guidance for local evaluation and keeps analysis aligned with the official metric (positive-class F1).
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    \toprule
    Gold label $\backslash$ Predicted label & 0 (No PCL) & 1 (PCL) \\
    \midrule
    0 (No PCL) & 1805 & 90 \\
    1 (PCL) & 65 & 134 \\
    \bottomrule
  \end{tabular}
  \caption{Confusion matrix on official dev for the final ensemble.}
  \label{tab:error_confusion_dev}
\end{table}

\textbf{What the error profile shows:}
The model is strong at ruling out negatives (TN \(=1805\)) but still misses a meaningful number of positives (FN \(=65\)). This produces precision \(=0.5982\), recall \(=0.6734\), and positive-class F1 \(=0.6336\). In other words, performance is now above baseline, but the main remaining ceiling is recovering subtle positives without creating too many additional false positives.
